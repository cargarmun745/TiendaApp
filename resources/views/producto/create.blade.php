  ...
  <div class="row">
      <div class="card card-plain">
        <div class="card-header">
            ...
            <h6 class="text-white text-capitalize ps-3">Creación de productos</h6>
          </div>
        <!--</div>-->
        <div class="card-body">
	            <form class="form-horizontal" action="{{ url('producto') }}" method="post" enctype="multipart/form-data">
        			@csrf
					<fieldset>
						
						<div class="input-group input-group-outline mb-3">
               <label class="form-label" for="customFile">Default file input example</label>
               <input type="file" name='fotoProducto' id='fotoProducto' class="form-control" id="customFile" />
            </div>
						...
						<div class="input-group input-group-outline mb-3">
							...
							<div class="col-md-9">
								<input value="{{ old('precio') }}" id="precio" name="precio" type="number" placeholder="Precio" class="form-control" step="0.01">
								@error('precio')
						            <div class="alert alert-danger">{{ $message }}</div>
						        @enderror
							</div>
						...
						<div class="input-group input-group-outline mb-3">
							...
							<div class="col-md-9">
								<input value="{{ old('unidadesDisponibles') }}" id="unidadesDisponibles" name="unidadesDisponibles" type="number" placeholder="Unidades disponibles" class="form-control">
						...
						<div class="input-group input-group-outline mb-3">
							...
							<div class="col-md-9">
								<select name="IDTipo" class="form-control">
									<option value=""  @if (old('IDTipo') == "") selected @endif>&nbsp;</option>
						            @foreach ($tipos as $tipo)
						                    <option value="{{ $tipo->id }}" @if(old('IDTipo') == $tipo->id) selected @endif >{{ $tipo->nombre }}</option>
						            @endforeach
						        </select>
								@error('IDTipo')
						            <div class="alert alert-danger">{{ $message }}</div>
						...
						<div class="input-group input-group-outline mb-3">
							...
							<div class="col-md-9">
								<select name="IDMarca" class="form-control">
						            <option value="" selected="true"  @if (old('IDMarca') == "") selected @endif>&nbsp;</option>
						            @foreach ($marcas as $marca)
						                    <option value="{{ $marca->id }}" @if(old('IDMarca') == $marca->id) selected @endif >{{ $marca->nombre }}</option>
						            @endforeach
						        </select>
								@error('IDMarca')
						            <div class="alert alert-danger">{{ $message }}</div>
